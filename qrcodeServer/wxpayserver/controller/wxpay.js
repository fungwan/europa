...
WXPay.mix('createUnifiedOrder', function (opts, fn) {
	...
	request({
		...
		util.parseXML(body, function (err, data) {
			var rtobj = _processerr(err,data);
			fn(rtobj.err, rtobj.data);
		});
	});
});
...
/*private fn*/
var _processerr = function(err,data){
	if (err) {
		err.code = errorType.wxPayError.xml; err.message = errorType.wxPayError.xml;
	} else {
		try {
			err = err || {};
			if (data.return_code == errorType.FAIL) {//通信标识结果判断，比如openid,appid,mchid是否匹配
				err.code = errorType.wxPayError.params;
				err.message = data.return_msg;
			} else if (data.result_code == errorType.FAIL) {//业务结果判断，比如商户订单号是否重复
				err.code = data.err_code;
				err.message = data.err_code_des;
			}
		} catch (error) {
			err.code = errorType.wxPayError.unkow;
			err.message = error.message;			
		}
	}
	err.code == undefined ? err = null : (err,logger.error('micro','wxpay throw error msg:'+err.message));

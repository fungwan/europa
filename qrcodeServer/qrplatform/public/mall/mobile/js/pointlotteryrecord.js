...
var selfgrade={
    renderList:function(data){
        ...
        for(var i=0;i<d.length;i++){
            $(".list-container").append(`<div class=" row no-gutter" style="padding-left: 1rem;">
                    <div class="col-25">${d[i].productname}</div>
                    <div class="col-25">${d[i].productnumber}</div>
                    <div class="col-25">${d[i].usepoint}</div>
                    <div class="col-25">${d[i].rectime.slice(0,10)}</div>
                </div>`)
        }
    ...
    initmypage:function(){
        ...
        app.scollItem.addItems('/lotto/getlottorecord',{
                only:true,
